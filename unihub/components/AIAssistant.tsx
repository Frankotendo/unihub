...
const AIAssistant: React.FC<AIAssistantProps> = ({ products }) => {
  ...
  const handleGenerateAd = async () => {
    ...
    try {
      ...
      setError("Failed to generate ad copy.");
    }
    setLoading(false);
  };
  ...
  const handleGenerateFlyer = async () => {
    ...
    try {
      ...
      if (url) {
        ...
        setError("The AI was unable to generate an image for this prompt. Try a different item.");
      }
    } catch (e) {
  ...
  const handleResearchTrends = async () => {
    ...
    try {
      ...
      setError("Could not complete market research.");
    }
    setLoading(false);
  };
  ...
  const handleOptimizePrice = async () => {
    ...
    try {
      ...
      setError("Strategy calculation error.");
    }
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      {error && (
        <div className="max-w-md mx-auto bg-rose-50 border border-rose-100 p-4 rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-8 mb-10">
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {loading && !adText ? <i className="fas fa-spinner fa-spin"></i> : 'Text Ad'}
                  </button>
                  <button 
                  ...
                  >
                    {loading && !flyerUrl ? <i className="fas fa-spinner fa-spin"></i> : 'Image Flyer'}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Text Ad Display */}
              <div className={`p-8 rounded-[2.5rem] border-2 border-dashed flex flex-col min-h-[300px] ${adText ? 'bg-indigo-50/30 border-indigo-200' : 'bg-slate-50 border-slate-200 items-center justify-center'}`}>
                {adText ? (
              ...
              {/* Flyer Image Display */}
              <div className={`p-8 rounded-[2.5rem] border-2 border-dashed flex flex-col min-h-[300px] ${flyerUrl ? 'bg-emerald-50/30 border-emerald-200' : 'bg-slate-50 border-slate-200 items-center justify-center'}`}>
                {loading && !flyerUrl ? (
                  <div className="text-center">
                    <i className="fas fa-palette fa-spin text-3xl text-emerald-500 mb-4"></i>
