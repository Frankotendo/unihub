    ...
    <div className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
           ...
           <section className="bg-white p-6 md:p-8 rounded-3xl md:rounded-[3rem] shadow-lg border border-slate-50">
              ...
              <div className="space-y-3">
                 {hubLogistics.map((log, idx) => (
                   <div key={log.id} style={{animationDelay: `${idx * 100}ms`}} className="group bg-slate-50 p-4 md:p-5 rounded-2xl border border-transparent hover:border-indigo-100 hover:bg-white transition-all animate-in slide-in-from-left-4">
                      <div className="flex items-center gap-4">
                         ...
                         <a href={`tel:${log.contact}`} className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-emerald-500 shadow-sm opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
           ...
           <section className="bg-[#0f172a] p-6 md:p-8 rounded-3xl md:rounded-[3rem] shadow-2xl text-white">
              ...
              <div className="space-y-3">
                 {hubVendors.map((v, idx) => (
                   <div key={v.id} style={{animationDelay: `${idx * 150}ms`}} className="bg-white/5 p-4 md:p-5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all animate-in slide-in-from-left-4">
                      <div className="flex items-center gap-4">
                         ...
                         <a href={`tel:${v.contact}`} className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white/50 hover:text-white transition-colors">
                            <i className="fas fa-phone-alt text-[10px]"></i>
                         </a>
                      </div>
                   </div>
        ...
        <div className="lg:col-span-8">
           <div className="bg-white rounded-[2rem] md:rounded-[4rem] shadow-xl border border-slate-100 overflow-hidden h-full flex flex-col min-h-[400px]">
              <div className="px-6 md:px-10 py-6 md:py-8 border-b border-slate-50 flex items-center justify-between">
                 <h4 className="text-[10px] md:text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-3">
                    <i className="fas fa-warehouse text-indigo-500"></i>
                    {activeHub} Stock
                 </h4>
                 <span className="bg-emerald-50 text-emerald-600 px-3 md:px-4 py-1.5 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest shrink-0">
                    {hubProducts.length} Listed
                 </span>
              </div>
              <div className="p-6 md:p-10 flex-1 overflow-y-auto no-scrollbar grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                 {hubProducts.map((p, idx) => (
                   <div key={p.id} style={{animationDelay: `${idx * 100}ms`}} className="bg-slate-50 p-5 md:p-6 rounded-3xl md:rounded-[2.5rem] border border-transparent hover:border-indigo-100 hover:bg-white transition-all group animate-in zoom-in duration-300">
                      <div className="flex gap-4 md:gap-6 items-center">
                         <div className="w-16 h-16 md:w-20 md:h-20 bg-white rounded-[1.2rem] md:rounded-[1.5rem] shadow-sm flex items-center justify-center text-slate-300 overflow-hidden shrink-0">
                            {p.imageUrl ? <img src={p.imageUrl} className="w-full h-full object-cover" /> : <i className="fas fa-box text-xl"></i>}
                         </div>
                         <div className="flex-1 overflow-hidden">
                            <span className="text-[8px] font-black text-indigo-500 uppercase tracking-widest mb-1 block truncate">{p.category}</span>
                            <h5 className="font-black text-slate-800 uppercase text-[11px] md:text-xs line-clamp-1 mb-2">{p.name}</h5>
                 ...
                 {hubProducts.length === 0 && (
                    <div className="col-span-full py-20 text-center opacity-30 flex flex-col items-center">
                       ...
                       <p className="text-[10px] font-black uppercase tracking-[0.4em]">Hub Empty</p>
                    </div>
                 )}
              </div>
           </div>
      ...
      {showLogModal && (
        <div className="fixed inset-0 bg-[#0f172a]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] md:rounded-[3rem] shadow-2xl w-full max-w-md p-8 md:p-10 animate-in zoom-in">
            <h3 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-tighter mb-6 italic">Add Rider</h3>
            <form onSubmit={handleAddLog} className="space-y-4 md:space-y-6">
              <input type="text" required className="w-full px-6 md:px-8 py-4 md:py-5 rounded-2xl bg-slate-50 border-none outline-none font-bold" placeholder="Rider Name" value={newLog.name || ''} onChange={e => setNewLog({...newLog, name: e.target.value})} />
              <input type="tel" required className="w-full px-6 md:px-8 py-4 md:py-5 rounded-2xl bg-slate-50 border-none outline-none font-bold" placeholder="WhatsApp Number" value={newLog.contact || ''} onChange={e => setNewLog({...newLog, contact: e.target.value})} />
              <button type="submit" className="w-full bg-indigo-600 text-white font-black py-4 md:py-5 rounded-[1.5rem] md:rounded-[2rem] uppercase text-[9px] md:text-[10px] tracking-widest shadow-xl">Link Rider</button>
              <button type="button" onClick={() => setShowLogModal(false)} className="w-full text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest py-2">Close</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showVendorModal && (
        <div className="fixed inset-0 bg-[#0f172a]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] md:rounded-[3rem] shadow-2xl w-full max-w-md p-8 md:p-10 animate-in zoom-in">
            <h3 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-tighter mb-6 italic">Link Vendor</h3>
            <form onSubmit={handleAddVendor} className="space-y-4 md:space-y-6">
              <input type="text" required className="w-full px-6 md:px-8 py-4 md:py-5 rounded-2xl bg-slate-50 border-none outline-none font-bold" placeholder="Shop Name" value={newVendor.name || ''} onChange={e => setNewVendor({...newVendor, name: e.target.value})} />
              <input type="tel" required className="w-full px-6 md:px-8 py-4 md:py-5 rounded-2xl bg-slate-50 border-none outline-none font-bold" placeholder="WhatsApp Number" value={newVendor.contact || ''} onChange={e => setNewVendor({...newVendor, contact: e.target.value})} />
              <input type="text" required className="w-full px-6 md:px-8 py-4 md:py-5 rounded-2xl bg-slate-50 border-none outline-none font-bold" placeholder="Specialty (e.g. Shoes, Gadgets)" value={newVendor.specialty || ''} onChange={e => setNewVendor({...newVendor, specialty: e.target.value})} />
              <button type="submit" className="w-full bg-emerald-600 text-white font-black py-4 md:py-5 rounded-[1.5rem] md:rounded-[2rem] uppercase text-[9px] md:text-[10px] tracking-widest shadow-xl">Link Partner</button>
              <button type="button" onClick={() => setShowVendorModal(false)} className="w-full text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest py-2">Close</button>
            </form>
          </div>
        </div>
      )}
