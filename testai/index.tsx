...
const App: React.FC = () => {
  ...
  const cancelRide = async (nodeId: string, actor: 'passenger' | 'driver' | 'admin') => {
    const node = nodes.find(n => n.id === nodeId);
    if (!node) return;
    ...
    try {
      if (node.assignedDriverId) {
        ...
        // RULE: Strict Dispatch Lock
        // If the trip is dispatched, neither driver nor passenger can cancel.
        // This prevents drivers from cancelling to get a refund or passengers from cancelling last minute.
        // Only Admin can cancel a dispatched trip, which triggers a refund to the driver.
        if (node.status === 'dispatched') {
             if (actor !== 'admin') {
                 alert("Ride is Active! Only Dispatch Control (Admin) can cancel this trip now. Please contact Support.");
                 return;
             }

             // Admin initiated cancellation of dispatched ride -> Refund Driver
             let refundAmount = 0;
             ...
             // Admin always deletes the node to clear it.
             await supabase.from('unihub_nodes').delete().eq('id', nodeId);
             alert("Ride terminated by Admin. Commission refunded to partner.");
             return;
        }

        // Handling Pre-Dispatch (Forming/Qualified) - Open for cancellation
        if (isBroadcast || (node.status === 'forming' && node.passengers.length === 0)) {
            // Driver cancelling their own empty broadcast
            // If they paid up-front (shuttle), refund them.
            if (isShuttle && isBroadcast) {
                 const cap = node.capacityNeeded;
                 const rate = settings.shuttleCommission || settings.commissionPerSeat;
        ...
        // Standard ride request cancellation (Pre-Dispatch)
        // If it's not dispatched, driver hasn't paid commission (except shuttle broadcast handled above).
        if (actor === 'passenger' || actor === 'admin') {
        ...
        // No driver assigned. Safe to delete by anyone.
        const { error: deleteErr } = await supabase.from('unihub_nodes').delete().eq('id', nodeId);
        if (deleteErr) throw deleteErr;
        removeRideFromMyList(nodeId);
        alert("Ride request removed.");
  ...
  const aiActions = {
     ...
     onNavigate: (target: string, sub_section?: string, modal?: string) => {
        ...
        else if (modal === 'help') setShowHelpModal(true);
        else if (modal === 'about') setShowAboutModal(true);
        else if (modal === 'close') {
        ...
        // Handle Views
        if (target === 'ride' || target === 'passenger') safeSetViewMode('passenger');
        else if (target === 'drive' || target === 'driver') safeSetViewMode('driver');
        else if (target === 'admin') safeSetViewMode('admin');

        // Handle Sub-sections (Tabs)
        if (sub_section) {
            if (viewMode === 'admin' || target === 'admin') {
                setActiveTab(sub_section); // Admin uses activeTab
            } else if (viewMode === 'driver' || target === 'driver') {
                setDriverTab(sub_section as any); // Driver uses driverTab
            }
        }
     }
  };
