    ...
    <div className="space-y-6">
            ...
            {myRides.map((node: RideNode) => {
              ...
              <div key={node.id} className="glass p-5 rounded-[2rem] border border-indigo-500/30 bg-indigo-900/10 relative overflow-hidden">
                 ...
                 {node.assignedDriverId && myPin && (
                    <div className="bg-black/30 p-4 rounded-xl mb-4 flex items-center justify-between gap-4 border border-white/5">
                       <div>
                          ...
                          <button onClick={() => setExpandedQr(myPin)} className="mt-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-[9px] font-black uppercase flex items-center gap-2 border border-white/10 transition-colors">
                             <i className="fas fa-expand"></i> Show Pass
                          </button>
                       </div>
                 ...
                 <div className="flex gap-2">
                    ...
                    {/* Disable LEAVE/CANCEL if dispatched */}
                    ...
                    {node.status === 'dispatched' && (
                        <div className="w-full py-3 bg-black/20 text-slate-500 rounded-xl font-black text-[9px] uppercase text-center border border-white/5">
       ...
       {expandedQr && (
         <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-6" onClick={() => setExpandedQr(null)}>
            <div className="glass-bright p-0 rounded-[2.5rem] w-full max-w-sm overflow-hidden animate-in zoom-in relative shadow-2xl border border-white/10" onClick={e => e.stopPropagation()}>
               <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-center relative overflow-hidden">
                   ...
                   <h3 className="text-xl font-black uppercase text-white tracking-widest relative z-10">Boarding Pass</h3>
                   <p className="text-[10px] font-bold text-indigo-200 uppercase tracking-widest relative z-10">NexRyde Secure</p>
               </div>
               
               <div className="p-8 bg-white flex flex-col items-center gap-6">
                   <div className="p-2 border-2 border-dashed border-slate-300 rounded-2xl">
                      <img src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${expandedQr}`} className="w-48 h-48" alt="Large QR" />
                   </div>
                   <div className="text-center">
                       <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Secure PIN</p>
                       <p className="text-4xl font-black text-[#020617] tracking-[0.3em] font-mono">{expandedQr}</p>
                   </div>
                   <p className="text-[9px] font-bold text-rose-500 uppercase bg-rose-50 p-2 rounded-lg w-full text-center">Show to partner upon boarding</p>
               </div>
               
               <button onClick={() => setExpandedQr(null)} className="absolute top-4 right-4 w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-white hover:bg-white/20"><i className="fas fa-times"></i></button>
            </div>
         </div>
       )}
    </div>
                  ...
                  {myActiveRides.map((node: any) => (
                      <div key={node.id} className="glass p-6 rounded-[2.5rem] border border-indigo-500/30 bg-indigo-900/10 relative overflow-hidden">
                          <div className="relative z-10">
                              ...
                              <div className="text-center">
                                  {/* Restrict Cancel to pre-dispatch only. Once dispatched, rely on admin to prevent refund loops */}
                                  ...
                                  <p className="text-[9px] text-rose-500 font-bold uppercase mt-2">Request Cancel</p>
                              </div>
                          </div>
                      </div>
                  ))}
              ...
              {nodes.map((n: any) => (
                  <div key={n.id} className="glass p-4 rounded-2xl border border-white/5 relative overflow-hidden">
                      ...
                      <div className="flex gap-2 mt-2">
                          {n.status !== 'completed' && (
                             <>
                               {/* Admin can always cancel any ride, including dispatched */}
                               <button onClick={() => onCancelRide(n.id)} className="flex-1 py-2 bg-rose-500/10 text-rose-500 rounded-lg text-[9px] font-black uppercase hover:bg-rose-500 hover:text-white transition-all">Cancel & Refund</button>
