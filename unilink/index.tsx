...
const App: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const { data: s, error: sError } = await supabase.from('settings').select('*').single();
      ...
      setConnectionError(err.message || "Network Error: Supabase project unreachable.");
    }
  }, []);
  ...
  if (connectionError) return (
    <div className="h-screen bg-[#020617] flex flex-col items-center justify-center p-8 text-center space-y-6">
       <div className="w-20 h-20 bg-rose-500/10 rounded-full flex items-center justify-center text-rose-500 border border-rose-500/20 animate-pulse">
          <i className="fas fa-wifi-slash text-3xl"></i>
       </div>
       <h1 className="text-2xl font-black uppercase italic tracking-tighter text-white">Hub Connection Lost</h1>
       <p className="text-slate-400 max-w-xs text-sm italic">{connectionError}</p>
       <button onClick={fetchData} className="px-8 py-3 bg-amber-500 text-black font-black uppercase text-[10px] rounded-xl shadow-lg shadow-amber-500/20">Re-establish Link</button>
    </div>
  );
  ...
  if (!settings) return (
    <div className="h-screen bg-[#020617] flex items-center justify-center">
       <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-amber-500 font-black tracking-widest text-[10px] uppercase italic animate-pulse">Syncing Hub...</p>
       </div>
    </div>
  );
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden lg:flex gap-2">
               <button onClick={() => setShowQrModal(true)} className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-amber-500 hover:bg-white/10 transition-colors"><i className="fas fa-qrcode"></i></button>
               <button onClick={() => setShowHelpModal(true)} className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-indigo-400 hover:bg-white/10 transition-colors"><i className="fas fa-circle-question"></i></button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
       <div className="flex justify-between items-end">
          ...
          <button onClick={() => setShowModal(true)} className="px-10 py-4 bg-amber-500 text-black rounded-2xl font-black text-xs uppercase shadow-xl hover:scale-105 active:scale-95 transition-all">Form Ride</button>
       </div>
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((node: any) => (
            <div key={node.id} className="glass p-8 rounded-[2.5rem] border border-white/5 space-y-6 text-left hover:border-white/10 transition-all">
               ...
               <div className="flex justify-between items-center pt-4 border-t border-white/5">
                  <div className="flex -space-x-2">
                     ...
                     {Array.from({length: Math.max(0, 4 - node.passengers.length)}).map((_, i) => <div key={i} className="w-10 h-10 bg-white/5 border-4 border-[#020617] rounded-xl flex items-center justify-center text-[10px] font-black opacity-20"><i className="fas fa-chair"></i></div>)}
                  </div>
          ...
          {filtered.length === 0 && <div className="col-span-full py-20 text-center opacity-10 font-black italic uppercase text-xs">No active nodes in this route</div>}
       </div>
       {showModal && (
          <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
             <div className="glass-bright w-full max-w-md p-8 rounded-[2.5rem] space-y-6 border border-white/10 animate-in zoom-in duration-300">
                <h3 className="text-xl font-black italic text-center">New Ride Node</h3>
                <input className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-amber-500" placeholder="Origin" onChange={e => setForm({...form, origin: e.target.value})} />
                <input className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-amber-500" placeholder="Destination" onChange={e => setForm({...form, destination: e.target.value})} />
                <input className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-amber-500" placeholder="Leader Name" onChange={e => setForm({...form, leader: e.target.value})} />
                <input className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-amber-500" placeholder="Phone" onChange={e => setForm({...form, phone: e.target.value})} />
                <div className="flex gap-4">
                   ...
                   <button onClick={async () => {
                     ...
                     setShowModal(false);
                   }} className="flex-1 py-4 bg-amber-500 text-black rounded-xl font-black text-[10px] uppercase shadow-xl active:scale-95 transition-transform">Launch</button>
                </div>
             </div>
          </div>
       )}
  ...
  if (!activeDriver) return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-8 animate-in zoom-in duration-500">
       ...
       <div className="w-full max-w-xs space-y-4">
          <select className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white font-bold outline-none cursor-pointer" onChange={e => setId(e.target.value)}>
             <option value="">Select Profile</option>
             {drivers.map((d:any) => <option key={d.id} value={d.id}>{d.name}</option>)}
          </select>
          <input type="password" placeholder="PIN" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-center text-xl font-black outline-none focus:border-indigo-500" onChange={e => setPin(e.target.value)} />
          <button onClick={() => onLogin(id, pin)} className="w-full py-4 bg-indigo-600 rounded-xl font-black text-[10px] uppercase shadow-xl active:scale-95 transition-transform">Sign In</button>
       </div>
    </div>
  );
  ...
  return (
    <div className="space-y-12 text-left animate-in fade-in duration-500">
       <div className="flex justify-between items-center bg-indigo-600/10 p-8 rounded-[2.5rem] border border-indigo-500/20">
          ...
          <button onClick={onLogout} className="px-6 py-3 text-rose-500 font-black text-[9px] uppercase border border-rose-500/20 rounded-xl hover:bg-rose-500/10 transition-colors">Exit</button>
       </div>
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <section className="space-y-4">
             ...
             {qualifiedNodes.map((n:any) => (
               <div key={n.id} className="glass p-6 rounded-2xl flex justify-between items-center border border-white/5 hover:border-white/10 transition-all">
                  <p className="font-black text-sm">{n.origin} → {n.destination}</p>
                  <button onClick={() => onAccept(n.id, activeDriver.id)} className="px-6 py-2 bg-indigo-600 rounded-lg text-[9px] font-black uppercase shadow-lg active:scale-95 transition-transform">Accept</button>
               </div>
            ))}
            {qualifiedNodes.length === 0 && <div className="p-20 text-center opacity-10 font-black italic uppercase text-xs border border-dashed border-white/10 rounded-2xl">Waiting for nodes...</div>}
          </section>
          <section className="space-y-4">
             ...
             {dispatchedNodes.filter((n:any) => n.assigned_driver_id === activeDriver.id).map((n:any) => (
               <div key={n.id} className="glass p-8 rounded-[2rem] border border-amber-500/20 space-y-6 animate-in slide-in-from-right-4">
                  <p className="text-xl font-black italic uppercase leading-none">{n.destination}</p>
                  <input className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-center text-4xl font-black outline-none focus:border-amber-500" placeholder="CODE" id={`v-${n.id}`} />
                  <button onClick={() => onVerify(n.id, (document.getElementById(`v-${n.id}`) as HTMLInputElement).value)} className="w-full py-4 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase shadow-xl active:scale-95 transition-transform">Complete Mission</button>
               </div>
            ))}
            {dispatchedNodes.filter((n:any) => n.assigned_driver_id === activeDriver.id).length === 0 && <div className="p-20 text-center opacity-10 font-black italic uppercase text-xs">No active missions</div>}
          </section>
       </div>
    </div>
  );
...
const AdminPortal = ({ activeTab, setActiveTab, drivers, topupRequests, hubRevenue, onApprove, onLock, settings, nodes }: any) => {
  ...
  const runAiSync = async () => {
    ...
    } catch (e) { setAiAnalysis('Intel link failed. Check API key configuration.'); }
    setAiLoading(false);
  };
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 text-left">
       <div className="flex gap-4 overflow-x-auto no-scrollbar py-2">
          ...
          <button onClick={onLock} className="px-6 py-3 text-rose-500 font-black text-[9px] uppercase border border-rose-500/20 rounded-xl hover:bg-rose-500/10 transition-colors">Lock</button>
       </div>
       ...
       {activeTab === 'monitor' && (
          <div className="space-y-8">
             ...
             <div className="glass p-10 rounded-[3rem] border border-indigo-500/20 space-y-6">
                ...
                <button onClick={runAiSync} className="px-8 py-3 bg-indigo-600 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-transform disabled:opacity-50" disabled={aiLoading}>Update Intel</button>
             </div>
          </div>
       )}
       ...
       {activeTab === 'requests' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {topupRequests.filter((r:any) => r.status === 'pending').map((r:any) => {
               ...
               return (
                  <div key={r.id} className="glass p-8 rounded-[2.5rem] border border-white/5 space-y-6 animate-in zoom-in">
                     ...
                     <button onClick={() => onApprove(r.id)} className="w-full py-4 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase shadow-xl active:scale-95 transition-transform">Approve</button>
                  </div>
               );
             })}
             {topupRequests.filter((r:any) => r.status === 'pending').length === 0 && <div className="col-span-full py-20 text-center opacity-10 font-black italic uppercase text-xs">No pending wallet requests</div>}
          </div>
       )}
             ...
             <div className="glass rounded-[2rem] overflow-hidden border border-white/5">
                <table className="w-full text-left text-[11px]">
                   ...
                   <tbody className="divide-y divide-white/5">
                      {drivers.map((d:any) => (
                         <tr key={d.id} className="hover:bg-white/5 transition-colors"><td className="px-8 py-5 text-white font-bold">{d.name}</td><td className="px-8 py-5 text-slate-500 uppercase">{d.license_plate}</td><td className="px-8 py-5 font-black text-amber-500 tracking-[0.2em]">{d.pin}</td><td className="px-8 py-5 text-emerald-400 font-black">₵{Number(d.wallet_balance).toFixed(1)}</td></tr>
                      ))}
                   </tbody>
                </table>
             </div>
...
const QrModal = ({ onClose }: any) => (
  <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
     <div className="glass-bright w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in duration-300 text-center border border-white/10">
        ...
        <button onClick={onClose} className="w-full py-4 bg-white/5 rounded-xl text-slate-500 font-black uppercase text-[10px] hover:text-white transition-colors">Close</button>
     </div>
  </div>
);
...
const HelpModal = ({ onClose }: any) => (
  <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
     <div className="glass-bright w-full max-w-2xl rounded-[3rem] p-10 lg:p-14 space-y-10 animate-in zoom-in duration-300 text-left border border-white/10 overflow-y-auto max-h-[90vh] no-scrollbar">
        <div className="flex justify-between items-center">
           <h3 className="text-3xl font-black italic uppercase text-white">Hub Guide</h3>
           <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-500 hover:text-white"><i className="fas fa-times"></i></button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
           ...
           <HelpSection icon="fa-id-card-clip" title="Drivers" points={["Top up your wallet via Admin Command.","Each mission costs commission set by admin.","Verify passenger codes to close missions."]} />
        </div>
        <button onClick={onClose} className="w-full py-4 bg-indigo-600 rounded-xl font-black text-xs uppercase shadow-xl active:scale-95 transition-transform">Understood</button>
     </div>
  </div>
);
...
const MobileNavItem = ({ active, icon, label, onClick, badge }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1 relative ${active ? 'text-amber-500' : 'text-slate-500'} active:scale-90 transition-transform`}>
...
const AdminLogin = ({ onLogin }: any) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-8 animate-in zoom-in duration-500">
       ...
       <button onClick={() => onLogin(val)} className="px-12 py-4 bg-amber-500 text-black rounded-xl font-black text-[10px] uppercase shadow-xl active:scale-95 transition-transform">Auth</button>
    </div>
  );
};
