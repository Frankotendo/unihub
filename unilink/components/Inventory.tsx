              ...
              <div className="space-y-6">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                       <input type="text" className="flex-1 px-5 py-3 rounded-xl bg-slate-50 border-none font-bold text-xs" placeholder="Location" value={newOption.location} onChange={e => setNewOption({...newOption, location: e.target.value})} />
                    ...
                    <div className="flex flex-wrap gap-2">
                       {deliveryOptions.map((opt, i) => (
                         <span key={i} className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
                            ...
                            <button onClick={() => removeDeliveryOption(i)}><i className="fas fa-times-circle"></i></button>
                         </span>
                       ))}
                    </div>
                 </div>
              ...
              <div className="space-y-8">
                 <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-2">
                       ...
                       <input type="text" required className="w-full px-8 py-5 bg-slate-50 rounded-2xl border-none font-bold text-slate-800 outline-none" value={newProduct.name || ''} onChange={e => setNewProduct({...newProduct, name: e.target.value})} />
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">Buying Cost</label>
                          <input type="number" required className="w-full px-8 py-5 bg-slate-50 rounded-2xl border-none font-bold" value={newProduct.sourcePrice || ''} onChange={e => setNewProduct({...newProduct, sourcePrice: Number(e.target.value)})} />
                       </div>
                       <div className="space-y-2">
                 ...
                 <button type="submit" className="w-full bg-[#0f172a] text-white py-6 rounded-3xl font-black text-xs uppercase tracking-[0.5em] shadow-2xl">Save To Hub Stock</button>
              </div>
           </form>
        </div>
      )}
           ...
           <div key={p.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 hover:shadow-2xl transition-all group">
              <div className="h-52 bg-slate-50 relative">
                 ...
                 <div className="absolute top-4 right-4">
                    <button onClick={() => onDelete(p.id)} className="w-10 h-10 bg-white/90 rounded-xl flex items-center justify-center text-rose-500 shadow-sm">
                      <i className="fas fa-trash-can text-xs"></i>
                    </button>
                 </div>
              </div>
              <div className="p-8">
                 <h4 className="font-black text-slate-800 uppercase text-xs truncate mb-2">{p.name}</h4>
                 ...
                 {activeTab === 'pending' && (
                    <button onClick={() => onApprove(p.id)} className="w-full mt-6 py-4 bg-emerald-600 text-white rounded-2xl font-black text-[9px] uppercase tracking-widest">Approve & Publish</button>
                 )}
              </div>
           </div>
         ))}
